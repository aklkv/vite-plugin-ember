...
## `setupEmber(app, options?)`

One-call setup for Ember live demos. Registers `<CodePreview>` and provides an owner for `@service` injection.
...
| Option          | Type                      | Default         | Description                                                     |
| --------------- | ------------------------- | --------------- | --------------------------------------------------------------- |
| `services`      | `Record<string, unknown>` | `{}`            | Services to register, keyed by name (without `service:` prefix) |
| `owner`         | `EmberOwner`              | `createOwner()` | A pre-built owner to use instead of creating one internally     |
| `componentName` | `string`                  | `'CodePreview'` | Name to register the component under                            |

### Returns

The `EmberOwner` instance, so you can register additional services later.
...
Pass a `services` map to `setupEmber` to make them available via `@service` in any live demo:
...
::: tip Advanced
If you need the owner instance directly (e.g. to register services lazily), `setupEmber` returns it:

```ts
const owner = setupEmber(app);
owner.register('service:auth', new AuthService());
```
